...
## Try without phylogeny on the intercept
phypriorsalt <- list(
    a_prior_mu = 30,
    a_prior_sigma = 10,
    b_zf_prior_mu = -4,
...
testmealt <- stan("stan/uber_threeslopesimpleintercept.stan",
    data=append(list(N=nrow(d), n_sp=nspecies, sp=d$sppnum,
       x1=d$force.z, x2 = d$chill.z, x3=d$photo.z,
       y=d$resp, Vphy=vcv(phylo, corr = TRUE)), phypriorsalt),
    iter=2000, chains=4,
    refresh = 10)

saveRDS(testmealt, "output/compactnophyint.rds")
...
# See also
# https://github.com/lizzieinvancouver/pmm/blob/5014539f8a7cfc659298d20d49a0935a8ced305d/analyses/phlyo_opsree_compact.R
fit <- readRDS("output/compact3.Rds")
...
# Zooom in on some of the issues ...

parszoom <- c("lam_interceptsa", "sigma_interceptsa", "lam_interceptsbf", "sigma_interceptsbf",
     "lam_interceptsbc", "sigma_interceptsbc", "lp__")

pdf(file="figures/ospreepairssubset.pdf")
pairs(~lam_interceptsa + sigma_interceptsa + lam_interceptsbf +  sigma_interceptsbf + 
    lam_interceptsbc +  sigma_interceptsbc + lp__, 
    data=rbind(nondiv_params[parszoom], div_params[parszoom]), pch=21, cex=0.8, bg=colz)
dev.off()
