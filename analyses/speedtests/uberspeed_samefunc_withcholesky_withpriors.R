...
## that has an old function and priors but no Cholesky decomposition
...
model_runs <- lapply(seeds, function(s) {
  ...
  # Fit model
  testme <- stan("try_seeds/stan/uberspeed_samefunc_withcholesky_withpriors.stan", # Or ubermini_2.stan
...
write.csv(mr_df, "try_seeds/output/uberspeed_samefunc_withcholesky_withpriors_compare.csv")
