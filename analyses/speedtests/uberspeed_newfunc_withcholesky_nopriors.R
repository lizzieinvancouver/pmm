...
## thats has a new function and Cholesky decomposition but no priors
...
model_runs <- lapply(seeds, function(s) {
  ...
  # Fit model
  testme <- stan("try_seeds/stan/uberspeed_newfunc_withcholesky_nopriors.stan", # Or ubermini_2.stan
...
write.csv(mr_df, "try_seeds/output/uberspeed_newfunc_withcholesky_nopriors_compare.csv")
