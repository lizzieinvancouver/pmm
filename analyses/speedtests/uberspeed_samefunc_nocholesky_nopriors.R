...
## thats has an old function, but no Cholesky decomposition and no priors
...
model_runs <- lapply(seeds, function(s) {
  ...
  # Fit model
  testme <- stan("try_seeds/stan/uberspeed_samefunc_nocholesky_nopriors.stan", # Or ubermini_2.stan
...
write.csv(mr_df, "try_seeds/output/uberspeed_samefunc_nocholesky_nopriors_compare.csv")
