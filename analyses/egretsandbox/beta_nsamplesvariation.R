...
nsamples <- c(5,10,25,50)
simulated_data <- lapply(nsamples, function(i){
  
  nind_species <- round(runif(nspecies, 1, i))
  n <- sum(nind_species)
...
temp <- future_lapply(simulated_data, function(simulated){
...
pdf(file=paste0("figures/nsamples_variation.pdf"))
par(mfrow = c(length(nsamples),4),
    mex=0.6, mar=c(5,4,3,2)+.1)
for(i in 1:length(nsamples)){
  plot_stuff(temp[[i]][["est"]], temp[[i]][["obs"]], temp[[i]][["sim"]], temp[[i]][["post"]], comments = paste0(', nmax.=', nsamples[i]))
}
dev.off()
