...
family_intercept <- rnorm(nfam, 0, param[["sigma_a"]])
...
dfhere <- data.frame(sp = c(), family = c(), fam_int = c(),  x1=numeric(), trait1=numeric())
for (i in 1:nfam){
  temp <- data.frame(sp = rep(i, nind),
                     family = rep(i, nind),
                     fam_int = rep(family_intercept[i], nind),
                     x1 = rnorm(n = nind, mean = 10, sd = 3),
                     trait1 = rep(slopes_b[i], nind))
  dfhere <- rbind(dfhere, temp)
}
dfhere$mu <-  dfhere$fam_int  + dfhere$x1 * dfhere$trait1
...
simu_inits <- function(chain_id) {
  b_z.temp <- rnorm(n = nfam, mean = param[["b_zf"]], sd = 1)
  return(append(list( b = b_z.temp),
                param))
}

test <- stan("..//stan/phylogeny_family_cholesky.stan", 
                 #  control = list(max_treedepth =15),
                 data = append(list(N=nrow(dfhere),
                                    Nfam= nfam,
                                    Nspp= nspecies,
                                    sppnum=dfhere$sp,
                                    famnum = unique(dfhere$family),
                                    x=dfhere$x1,
                                    y =dfhere$y,
