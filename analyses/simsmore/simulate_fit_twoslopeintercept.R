...
phypriors <- list(
    ...
    b_zc_prior_sigma = 1,
    lam_interceptsbc_prior_alpha = 9, #
...
dfhere$mu <- dfhere$intercept + dfhere$x1 * dfhere$trait1 + dfhere$x2 * dfhere$trait2
...
simu_inits <- function(chain_id) {
    ...
    b_zf.temp <- rnorm(n = nspecies, mean = param[["b_zf"]], sd = 1)
    b_zc.temp <- rnorm(n = nspecies, mean = param[["b_zc"]], sd = 1)
    return(append(list(a = a_z.temp, b_force = b_zf.temp, b_chill = b_zc.temp),
                  param))
}
