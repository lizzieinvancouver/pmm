...
// make_stancode of brmmod.wgc in run_MCMCbrms_pglmm.R 
...
transformed parameters { 
  ...
  // group-level effects 
  vector[N_2] r_2_1 = sd_2[1] * (Lcov_2 * z_2[1]);
} 
model { 
  ...
  target += student_t_lpdf(sd_2 | 3, 0, 20)
    - 1 * student_t_lccdf(0 | 3, 0, 20); 
...
generated quantities { 
  ...
  real prior_sd_2 = student_t_rng(3,0,20);
  ...
  while (prior_sd_2 < 0) {
    prior_sd_2 = student_t_rng(3,0,20);
  }
}
