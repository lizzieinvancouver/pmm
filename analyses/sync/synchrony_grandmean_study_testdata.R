...
sp_intercept <- rnorm(nspecies, 0, param[["sigma_a"]])
study_intercept <- rnorm(nstudy, 0, param[["sigma_study"]])
...
dfhere <- data.frame(sp = c(),study = c(), grand = c(), sp_int = c(), study_int = c(), x1=numeric(), trait1=numeric())
for (i in 1:nspecies){
  temp <- data.frame(sp = rep(i, nind),
                     study = rep(i, nind),
                     grand = param[["grandMean"]],
                     sp_int = rep(sp_intercept[i], nind),
                     study_int = rep(study_intercept[i], nind),
                     x1 = rnorm(n = nind, mean = 10, sd = 3),
                     trait1 = rep(slopes_b[i], nind))
  dfhere <- rbind(dfhere, temp)
}
dfhere$mu <- dfhere$grand + dfhere$sp_int + dfhere$study_int + dfhere$x1 * dfhere$trait1
...
summary(test)$summary[c("b_z", "lam_interceptsb","a_z", "lam_interceptsa","sigma_interceptsa", 'mu_study',"sigma_y"),"mean"]; t(param)
