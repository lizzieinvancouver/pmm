...
param <- list(mu_a = 4, # root value intercept
              sigma_a = 1,
              b_z = 0.9, # root value trait1 slope
...
# Generate intercept
intercepts <- rnorm(nspecies, 0, param[["sigma_a"]])
# Generate b slope
scaledtree_b <- rescale(spetree, model = "lambda", param[["lam_interceptsb"]])         
slopes_bf <- fastBM(scaledtree_bf, a = param[["b_zf"]], mu = 0, sig2 = param[["sigma_interceptsb"]] ^ 2)
...
test <- stan("stan/oneslope_unpooledint_testdata.stan", 
                 ...
                 warmup = 3000,
                 chains = 1)
summary(test)$summary[c("mu_a","sigma_a","b_z", "lam_interceptsb","sigma_interceptsb","sigma_y"),"mean"]; t(param)
